...
{
	//Did the window resize? if so we need to adjust our back buffer, so window resize 
	//We want the backbuffer to resize as well 
	if (sGraphicsSystem)
	{
		switch (message)
		{
...
//Give it a window , here are my window sizes now we want our back buffer to match that size
void GraphicsSystem::Initialize(HWND window, bool fullscreen)
{
	...
	//Device is the thing we ask from memory
	HRESULT hr = D3D11CreateDeviceAndSwapChain
	(
		nullptr,
		//Hardware abstraction layer, we want acceleration, we don't want the emulation. 
	...
	//aka What did you actually end up creating for me 
